...
import { Car, Building2, Trash2 } from 'lucide-react';

const AdminHotelCabsView = () => {
  ...
  const fetchCabs = async () => {
    try {
      ...
      setGroupedCabs(res.data.data.hotelCabs);
    } catch (err) {
      toast.error('Failed to fetch hotel cabs');
    } finally {
      setLoading(false);
    }
  };
  ...
  return (
    <div className="space-y-8">
      <div>
        <h2 className="text-2xl font-bold text-gray-900">Hotel Cabs</h2>
        <p className="text-gray-500">View and manage all active cabs associated with specific hotels on the platform.</p>
      </div>

      {groupedCabs.length === 0 ? (
        <div className="bg-white p-12 text-center rounded-xl shadow-sm border border-gray-100 text-gray-500">
          No hotel-linked cabs found.
        </div>
      ) : (
                ...
                <table className="w-full text-left text-sm whitespace-nowrap">
                  <thead className="bg-white text-gray-500 border-b border-gray-50">
                    <tr>
                      ...
                      <th className="py-3 px-6 font-medium">Driver Name</th>
                    ...
                    {group.cabs.map(cab => (
                      <tr key={cab._id} className="hover:bg-gray-50/50 transition">
                        ...
                        <td className="py-4 px-6 font-medium text-gray-700">
                          {cab.vendorId?.vendorDetails?.driverName || 'Not Assigned'}
                        </td>
                        <td className="py-4 px-6">
...
export default AdminHotelCabsView;
